...
/*
 * [목적] ContentBackfillJob 상태 전이(start/recordProgress/succeed/fail)를 REQUIRES_NEW 트랜잭션으로 격리 커밋.
 ...
 *       DisclosureContentBackfillService 내에서 직접 호출(this.startJob())하면 REQUIRES_NEW가 무시됨.
 *       별도 빈으로 분리해 DisclosureContentBackfillService에서 프록시를 경유하도록 강제.
 * [사이드 임팩트] 이 빈은 ContentBackfillJob 상태 쓰기에만 집중 — 읽기(findByJobId)는 호출자 책임.
 ...
 */
@Service
public class ContentBackfillJobStateService {

    private final ContentBackfillJobRepository jobRepository;

    public ContentBackfillJobStateService(ContentBackfillJobRepository jobRepository) {
