...
/*
 * [목적] 포트폴리오 CRUD 서비스 — Free 3종목 제한 + AES-256-GCM 암복호 + IDOR 방지.
...
public class PortfolioService {
    ...
    public PortfolioResponse createPortfolio(Long userId, PortfolioRequest request, Tier tier) {
        ...
        if (!stockRepository.existsByStockCode(request.stockCode())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "지원하지 않는 종목코드입니다");
        }
