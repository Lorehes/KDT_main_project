...
public interface ConsentLogRepository extends JpaRepository<ConsentLogEntity, Long> {
    ...
    /**
     * 필수 동의 타입 중 최신 기록이 agreed=true인 타입 수를 반환.
     * hasRequiredConsents() 전용 — 4개 타입 전체 로드 대신 필요 타입만 조회하여 DB I/O 최소화.
     * 반환값 == types.size() 이면 모든 필수 동의 완료.
     */
    @Query("""
            SELECT COUNT(DISTINCT c.consentType) FROM ConsentLogEntity c
            WHERE c.userId = :userId
              AND c.consentType IN :types
              AND c.agreed = true
              AND c.agreedAt = (
                    SELECT MAX(c2.agreedAt) FROM ConsentLogEntity c2
                    WHERE c2.userId = c.userId AND c2.consentType = c.consentType)
            """)
    long countAgreedRequiredConsents(@Param("userId") Long userId,
                                     @Param("types") List<ConsentLogEntity.ConsentType> types);
}
