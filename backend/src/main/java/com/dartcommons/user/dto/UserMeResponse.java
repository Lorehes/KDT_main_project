...
/*
 * [목적] GET /api/v1/users/me 응답 DTO — 사용자 프로필·알림 설정·BM 티어·휴대폰 인증 상태·투자 경험 정보.
 ...
 *               investment_experience / preferred_time: V22 신규. null = profile 단계 미입력.
 * [수정 시 고려사항] phoneNumber(복호화 값) 노출이 필요하면 별도 엔드포인트로 분리(보안 리뷰 필수).
 *                  investment_experience는 FE 개인화 표시 전용 — 투자 권유 판단에 활용 금지.
 */
public record UserMeResponse(
    ...
    public static UserMeResponse from(UserEntity u) {
        return new UserMeResponse(
                ...
                u.getCreatedAt(),
                u.getInvestmentExperience() != null ? u.getInvestmentExperience().name() : null,
                u.getPreferredTime()         != null ? u.getPreferredTime().name()         : null
        );
    }
}
