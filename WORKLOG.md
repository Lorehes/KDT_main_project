...
### 완료
- **서버 백필 중단 판단**: 첫 청크 실측 100건/19.5분(건당 11.7초 ≈ 총 13일) — Lightsail 2vCPU CPU 임베딩 한계 + 버스트 크레딧 소진 리스크 → backend 재시작으로 잡 중단(커서 보존).
- **로컬 백필**: jar 직접 실행(bootRun의 .env 자동 로드 우회, LLM=mock으로 OpenRouter 쿼터 보호) → 93,560건 4시간 55분(건당 0.19초), **실패 0**. jobId caab88bb.
- **이관**: 사전 검증(로컬·서버 `id:rcept_no` md5 완전 일치 e68d2985…) → 로컬 Chroma 정지 → tar 478MB → scp(md5 일치) → 서버 기존 디렉터리 백업(`.bak-20260706`) → 교체 → 재기동 → **서버 count=93,560 확인**.
- 운영가이드 §9-3 정정: admin API는 nginx 미노출(실측 401) — backend 컨테이너 내부 호출로 런북 수정.

### 결정
- 임베딩 백필은 **로컬 컴퓨트 + 데이터 이관**이 표준 경로 (서버 CPU 임베딩은 소량 증분만 감당). DB id 매핑 md5 일치 사전 검증이 이관 안전성의 전제.
- 로컬 backend 실행 시 bootRun 대신 jar + 명시적 env — .env가 서버 마스터 사본이라 bootRun 자동 로드는 서비스명(chroma:8000 등)이 로컬에서 깨짐.

### 미완료 → 다음 세션
- 서버 backend 재시작(옛 컬렉션 UUID 캐시 해제) + stale 잡(d27a5a83) FAILED 마킹 — 사용자 실행 대기.
- Pro 계정 상세 페이지 유사 공시 실데이터 확인(브라우저).
- 뒷정리(수일 후): 서버 tar·`.bak-20260706`, 로컬 `~/data/chroma-backfill-20260706.tgz` 삭제.
- 다음 마일스톤: Stage 4(LLM 최종)·Stage 5(재무/업황)·알림톡 채널·타입게이트 Tier2.
...
### 완료
- `disclosures/[id]/page.tsx`: `reactionCfg` 렌더 블록을 `inline-flex` 한 줄(배지+이유 인라인)에서 `flex-col gap-2`(배지 독립 한 줄, 이유 별도 `<p>`)로 분리. 글자 크기 변경 없음.

### 결정
- 이유(rationale) 텍스트가 배지 옆에 붙으면 줄바꿈 시 배지 레이블과 이유가 혼재해 가독성 저하 → 두 줄 구조로 명확히 분리.
