...
//   PublicMobileMenu(client)를 서브컴포넌트로 가져와 모바일 햄버거 기능 추가 — RSC 이점 유지.
// [수정 시 고려사항] isAuthenticated는 dr_session presence(httpOnly)라 서버에서만 판정 가능 → 반드시 prop으로 받음.
//   NAV_ITEMS 변경 시 PublicMobileMenu의 NAV_ITEMS도 동기화 필요.
    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-sm">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* 모바일 햄버거 */}
          <PublicMobileMenu isAuthenticated={isAuthenticated} />
        </div>
      </div>
    </header>
  );
